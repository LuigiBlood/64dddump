...
void fat_start(char diskIDstr[])
{
  ...
  sprintf(filenamestr, "NUD-%4s-JPN.ndd", diskIDstr);
  sprintf(filenamelogstr, "NUD-%4s-JPN.log", diskIDstr);
...
void fat_startlog(char diskIDstr[])
{
  ...
  sprintf(filenamestr, "NUD-%4s-JPN.log", diskIDstr);
...
{	
  ...
  while(1) {
    ...
    setcolor(0,255,0);
    draw_puts("\f\n    64DD dump v0.53 (Gray) by LuigiBlood & marshallh\n    ----------------------------------------\n");
    setcolor(255,255,255);
 ...
 //Uncomment these for Leo stuff and remove the line just above
    ...
    if (DISKID_READ == 0)
    {
    	...
    	switch (error)
    	{
    		case LEO_ERROR_COMMAND_TERMINATED:
    		case LEO_ERROR_GOOD:
    			sprintf(console_text, "    Disk ID: %4s\n    Press START to dump Disk (with skips, faster)\n    Press Z to dump every block (no skips, slower)", _diskID.gameName);
    			draw_puts(console_text);
    			DISKID_READ = 1;
    			break;
    		case LEO_ERROR_MEDIUM_NOT_PRESENT:
    ...
    if (((newbutton & START_BUTTON) || (newbutton & Z_TRIG)) && (DISKID_READ > 0) && isDiskPresent() == 1)
    {
      ...
      if (DISKID_READ == 1)
        ...
        for (selectLBA = 0; selectLBA < 0x10DC; selectLBA += 1)
        {
            ...
            //Prepare SKIP stuff
            if (selectLBA == 0)           //0 = Retail, 2 = Blue
            {
              osWritebackDCacheAll();
        ...
        if (errorsLBA > 0)
        {
          ...
          sprintf(console_text, "64DDdump 0.53 (Gray)\r\nNUD-%4s-JPN.ndd LOG\r\n---", _diskID.gameName);
